...
% Output (to GUI):
 ...
 % run simulation each time to evaluate if braking is needed inputs for
 % simulation are static left and right values and input speed based on
 % fitted model
 sim('KITBEST.slxc')
 %actual time of simulation in GUI
 t1 = toc;
 %getting data from the model
 stop = out.stop.data(:,1);
 t = out.speed.time;
 x = out.position.data(:,1);
 %finding the switching time for stopping with braking 
 for j=1:length(t)
     if (x(j) <= eps)
         tau = t(j);
     end
 end 
 if (stop == 1 && toc <= tau+t1 )
    EPOCommunications('transmit','M135');   % maximum braking for tau seconds
 elseif(stop == 1 && toc >= tau+t1)
    EPOcommunications('transmit','M150') % stopping the car after tau seconds
 end 
 xleftvector = [xleft Left];
 xrightvector = [xright Right];
 save('xleft');
 save('xright');
...
if stopCar == 1
    ...
    if x>=30 && x<=40
            if Left <= x+15+95-(velocity) || Right <= x+15+95-(velocity)
            EPOCommunications('transmit','D150');   % direction
        ...
        if Left <= x+20+95-(velocity) || Right <= x+20+95-(velocity)    % where we stop actively breaking and just stop the wheels: this is depedndent on the control system
            EPOCommunications('transmit','D150');   % direction
            EPOCommunications('transmit','M150');   % speed
            toc
            pause(1);
            stop(t);
            status = EPOCommunications('close',comport);
%     elseif Left <= x+95+velocity || Right <= x+95+velocity
%         EPOCommunications('transmit','D150');   % direction
