...
s = int2str(speed);
signal = ['M', s];
disp("Press 'R' to start a recording. The words 'Go', 'Ready' & 'Stop' are used.")
...
disp(['Left: ', int2str(Left), ' & Right: ', int2str(Right)])
...
% Breaking:
    if x>=30 && x<=40
            ...
            %toc
            pause(2);
            t_toc = sprintf('%.4f',toc);
            disp(['The car took', t_toc, 'seconds to break']);
            disp(['Left sensor: ', Left, '& right sensor: ', Right]);
            stop(t);
            status = EPOCommunications('close',comport);
            end
    else
